...
/**
 * Owns persisted workspace preferences such as language, notifications,
 * section order, and transient layout-edit state while keeping server-backed
 * preference reloads from clobbering an active layout edit session.
 */
export function useWorkspacePreferencesController({ language, pushNotification, session, setLanguage, withPending }) {
  ...
  async function persistUiPreferences(nextPreferences) {
    ...
    await withPending('uiPreferences', async () => {
      try {
        ...
        await ensureLocaleLoaded(normalized.language)
  ...
  async function applyLoadedUiPreferences(payload, userId) {
    ...
    await ensureLocaleLoaded(nextUiPreferences.language)
