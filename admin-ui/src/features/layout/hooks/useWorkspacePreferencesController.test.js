...
describe('useWorkspacePreferencesController', () => {
  ...
  async function applyLoadedPreferences(result, payload, userId = 42) {
    await act(async () => {
      await result.current.applyLoadedUiPreferences(payload, userId)
    })
  }

  it('normalizes loaded preferences including legacy user section ids', async () => {
    const { result } = renderController()

    await applyLoadedPreferences(result, {
      persistLayout: true,
  ...
  it('updates only the selected workspace quick setup visibility state', async () => {
    const { result } = renderController()
