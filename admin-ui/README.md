...
- the source-email-account and destination-mailbox dialogs now surface an explicit spam/junk strategy: folder discovery pre-fills editable source and destination spam/junk paths, sources can leave spam alone, import it normally, or route it to Gmail `SPAM` / the destination spam folder, and the UI keeps that decision visible instead of burying it in polling behavior
