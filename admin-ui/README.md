...
- chart resolution choices are now range-aware and match the real backend buckets: `Today` and `Yesterday` stay hourly, `Week` supports hour/day, `Month` supports day/week, `Trimester`, `Semester`, and `Year` support week/month, and `Custom` adapts its available resolutions to the selected span
