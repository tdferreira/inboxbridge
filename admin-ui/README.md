...
- that same Security `Sessions` tab now also includes a dedicated `Browser extensions` card, so the signed-in user can inspect existing extension installs, revoke stale browser-extension access later from the normal app session, and issue a manual fallback token if direct extension sign-in is unavailable for a specific browser profile
