...
- the source-email-account dialog now only shows `Custom Label` while the current destination mailbox is Gmail-backed or not configured yet; once the saved destination switches to a non-label IMAP target such as Outlook, that Gmail-only field is hidden instead of implying that IMAP APPEND destinations understand labels
