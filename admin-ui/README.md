...
- the full frontend Vitest run now uses forked workers with a default `50%` worker cap via `admin-ui/scripts/run-vitest.mjs` instead of forcing a single worker, while CI, Docker image builds, and coverage runs stay pinned to one worker for stability on stricter environments
