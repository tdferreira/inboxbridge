...
- the `/remote` surface now also applies the signed-in user's saved theme preference and exposes a lightweight Preferences dialog so that same account can switch between `Follow system`, `Light green`, `Light blue`, `Dark green`, and `Dark blue` without leaving the remote page
...
- when a `/remote` source is actively processing a known batch, the top summary pill already carries the determinate `Processing X / Y emails (size / size)` label and fill state, so the card should not repeat that same progress or the transient fetched/imported/duplicates summary in an extra live block below
...
- expanded source-account details in both `My Source Email Accounts` and `Administration` now surface mailbox/runtime diagnostics such as destination identity, POP UIDL checkpoint, per-folder IMAP checkpoints, persisted adaptive-throttle state, IMAP IDLE watcher health, the latest cooldown/throttle decision audit values from the last poll, and explicit runtime alert pills when InboxBridge detects disconnected IMAP IDLE watches, active cooldown loops, or sustained throttling
...
- the main Preferences dialog now also persists a per-user theme mode (`Follow system`, `Light green`, `Light blue`, `Dark green`, or `Dark blue`) so both the main admin UI and `/remote` can render the same account-specific palette choice consistently; legacy stored `LIGHT` / `DARK` values are treated as `Light green` and `Dark blue`
- the Preferences dialog now also lets each signed-in user choose a timezone mode: keep detecting the current browser timezone automatically, or pin the account to a manually selected IANA timezone such as `Europe/Lisbon` or `America/New_York`
- the Preferences dialog now also lets each signed-in user choose a date/time display format; `Automatic` follows the selected language's default pattern, while the effective timezone still controls which local time is shown
- manual date formats include the built-in preset list `DMY_24`, `DMY_12`, `MDY_24`, `MDY_12`, `YMD_24`, and `YMD_12`, plus a `Custom…` option that opens a validation dialog
- custom date patterns are stored directly in the single persisted `dateFormat` preference, show a live example before saving, and are then applied consistently across the main app, notifications, security timestamps, charts, and `/remote`
- supported custom date tokens currently include `YYYY`, `YY`, `MMMM`, `MMM`, `MM`, `DD`, `dddd`, `ddd`, `HH`, `H`, `hh`, `h`, `mm`, `M`, `ss`, `S`, and `A`
- when opening the custom date-format dialog, the input is prefilled from the currently selected date format so users can start from the active preset or locale-derived automatic pattern instead of an unrelated default
- the custom date-format dialog now keeps the supported-token helper text visible below the live example and offers a pillbox-style contextual token autocomplete while typing, or immediately on focus based on the caret position; after separators it shows all tokens, within a token fragment it filters from that fragment, and localized aliases such as Portuguese `AAAA` / `AA` are normalized back to the canonical stored syntax before save
- the `Automatic` date-format option now shows the effective locale-derived pattern in the picker itself, for example `Automatic (MM/DD/YYYY, hh:mm:ss A)` in US English or `Automatic (DD/MM/YYYY, HH:mm:ss)` in locale variants that default to 24-hour time
- when timezone mode is `Set manually`, the timezone selector is rendered before the date-format controls in Preferences so the chosen zone is already established before the user previews or edits manual date formatting
- all date/time rendering in `My InboxBridge`, `Administration`, Notifications Center, Security session details, and InboxBridge Go now follows that effective per-user timezone instead of always using the browser default, and the polling statistics requests reuse the same effective timezone for bucket labels
...
- that same Security `Sessions` tab now also includes a dedicated `Browser extensions` card, so the signed-in user can inspect existing extension installs, revoke one or all stale browser-extension sessions later from the normal app session, and collapse either the normal sessions card or the browser-extensions card independently when the tab gets crowded
- the Browser extensions card separates active extension access from recently revoked extension sessions, keeps the revoked history visible for 30 days, and disables `Revoke all extensions` once no extension still has access
...
- those new-sign-in warnings now also flag a session as unusual when its Geo-IP location appears to come from a country not seen in that account's recent session history; because this signal currently works from stored Geo-IP labels rather than precise login coordinates, the heuristic stays intentionally conservative and waits for enough prior history before warning
...
- those statistics views now show line-chart trends for imports, duplicates, and errors, along with provider breakdowns, source-account health buckets, run counts, and average poll duration
...
- source-email-account statistics now suppress deployment-wide account-health counters and instead show source-relevant values such as imported-message totals, error-poll counts, provider breakdown, and run activity
- when a source email account is configured for `IMAP IDLE`, its source-level run chart switches from the generic `Poll activity over time` title to `Source activity over time` and breaks the run timeline into manual, scheduled, and `Realtime source activations` lanes so realtime imports are not mislabeled as manual polling
- the shared statistics UI now explains that `Realtime source activations` count the initial IMAP IDLE sync plus later activations caused by newly arrived mail, not connection time
...
- polling statistics requests now send the signed-in viewer's effective IANA timezone in `X-InboxBridge-Timezone`, so hourly and daily chart buckets are labeled and grouped in that account's chosen local time instead of raw UTC; auto-detect follows the current browser timezone, while manual override keeps the same account aligned across different devices or regions
- changing that timezone preference now also triggers an immediate stats reload in the main workspace and admin dashboard, and later manual/background refreshes continue using the updated timezone instead of falling back to the previous one
...
- the source-email-account dialog now only shows Gmail label controls while the current destination mailbox is Gmail-backed or not configured yet; once the saved destination switches to a non-label IMAP target such as Outlook, the default label and folder-to-label mapping fields are hidden instead of implying that IMAP APPEND destinations understand labels
- the source-email-account and destination-mailbox dialogs now surface an explicit spam/junk strategy: folder discovery pre-fills editable multi-value source spam/junk folders plus the destination spam/junk path, sources can leave spam alone, import it normally, or route it to Gmail `SPAM` / the destination spam folder, and the UI keeps that decision visible instead of burying it in polling behavior
- the source-email-account dialog loads Gmail label IDs from the linked Gmail destination for the default-label picker, while keeping `INBOX` and `SPAM` as fallback options when label discovery is unavailable
- folder-to-label mappings in the source-email-account dialog are edited as structured source-folder/Gmail-label picker rows with removable pills; the UI still serializes them into the backend's `folder=label` format, but users should not have to type that syntax directly
- the reusable IMAP folder pillbox now keeps a single visible field on mobile too: once the dialog has a known folder list it always routes taps through an overlaid native multi-select picker instead of rendering a second visible select below the field, while still falling back to manual typed pills before the server folders are known
- when editing an existing IMAP source whose saved server settings are already sufficient, the dialog now loads the current remote folder list automatically so the folder pillbox can be enriched before the user manually clicks `Test Connection`
- the reusable pillbox now reports both focus and typing activity to outer flows; the source-account controller uses that signal to fetch IMAP folders once per dialog/server-signature state when the folder field first receives focus, retry later focuses after a failed fetch, and fall back to the first typed character only when the folders are still unknown
- the reusable pillbox keeps the same neutral border in idle and successful validated states; it only switches to a soft red error border when the caller marks the field invalid after a failed save/validation attempt, and that error chrome should clear again as soon as the user edits the field or the value becomes valid
- dialog-style surfaces such as the source editor, Notifications, Preferences, and security/settings modals now keep native scrolling but hide the browser scrollbar chrome so those overlays preserve the intended card layout while still supporting touch, wheel, and trackpad scrolling
- the add/edit source-email-account dialog now groups its mailbox-processing controls more intentionally: `After polling` and `Mark as read after polling` share one row, `Source update mode` sits above the `TLS` / `Unread only` row, and `Enabled` is the final toggle at the bottom of the form
...
- source-account transport security is now verified dynamically: the dialog still exposes the TLS toggle for legacy plain-text source servers, but disabling TLS shows an explicit unsafe-transport warning, a successful connection test auto-enables TLS when InboxBridge can verify either an implicit-TLS endpoint or STARTTLS/STLS support on the configured IMAP/POP3 port, and the toggle then stays locked so users cannot save a plain-text source while that encrypted path is known to exist
- source-account connection testing now reports connectivity first and loads IMAP folders afterward, so a folder-discovery timeout no longer leaves the `Test Connection` action spinning forever or overwrites a successful connection result
- the source-account dialog also blocks duplicate source definitions for the same user when the same server identity and effective credentials are already configured
...
- each source-email-account connection-test result label now uses an inline info icon with translated explanatory copy, including the best-effort `$Forwarded` capability probe, instead of rendering that forwarded-marker caveat as a long standalone paragraph below the results
...
- repeated identical error or warning notifications now aggregate into a single refreshed banner/history entry with an inline repeat count badge instead of stacking dozens of copies during connectivity or proxy-failure loops such as repeated `Load failed` or `502 Bad Gateway` refresh errors; the current grouping window is intentionally short so bursts are folded together without turning unrelated later incidents into one permanent notification
...
- the admin-only `Global Statistics` section now raises a red anomaly warning if its hourly scheduled-run timeline shows activity far above what the current effective polling interval would permit across the enabled source-mailbox count; the section pulses temporarily to grab attention, a linked notification is added only for administrators when the suspicious bucket is still within the last 24 hours, the static warning now ages out after one week so stale incidents do not keep presenting as active problems forever, and the poll-activity line chart marks the suspicious bucket directly with a warning glyph that exposes the translated anomaly explanation on hover
- timeline charts now avoid sticky active-point/crosshair selection, warning markers expose a brief hover-or-click callout with the anomaly explanation, and the toolbar is intentionally limited to `Range` plus `Resolution` so the chart state stays predictable instead of resetting around an interactive brush
- anomaly warning markers now anchor to the actual detected bucket for the current chart resolution, so an hourly anomaly still lands on the correct day or week slot after the chart aggregates the timeline
- those anomaly markers and warning messages now also retain the originating stats range, so a `Yesterday 07:00` spike no longer renders on the `Today` chart just because both ranges share the same hour label; the detector also collapses contiguous suspicious hourly buckets into a single anomaly window, anchors the marker at the start of that window, lets the admin jump the run chart straight to the affected range from the warning itself without making that focus sticky on later refreshes, focuses the chart card when `Show on chart` is clicked, overlays the scheduled-runs line with a red highlight during the detected anomaly window so the spike stands out visually, and falls back to localized bucket-label formatting instead of ever rendering `Unavailable` or raw ISO timestamps when a suspicious span cannot be resolved into a valid datetime window
- the shared timeline chart now preserves its selected range and manual resolution choice across ordinary stats refreshes, chart remounts, and same-session page refreshes for the same stats card, so background layout/data refreshes no longer snap a user back to a default view
- timeline refreshes now keep the currently selected stats range even if a later payload omits some previously available timeline keys, so a sparse backend refresh can no longer force the chart selector back to a default range
- chart resolution choices are now range-aware and match the real backend buckets: `Past day` and `Yesterday` stay hourly, `Week` supports hour/day, `Month` supports day/week, `Trimester`, `Semester`, and `Year` support week/month, and `Custom` adapts its available resolutions to the selected span
- the first preset range is now a rolling `Past day` window ending at the current hour, not calendar-today through midnight, so the line charts do not waste half the view on future empty hours
- custom ranges of up to one week now also allow hourly resolution, and once a custom range has been applied the selected date-time window stays visible in the chart toolbar using the signed-in user’s localized datetime format and can be edited again directly from an `Edit dates` action without having to leave the current chart view first
- hard-refresh chart restore now ignores stale persisted preset/custom ranges that no longer exist in the freshly loaded stats payload and falls back to the first preset with actual data instead of rendering an empty chart until the user changes the range manually
- the statistics custom-range dialog now supports both explicit start/end timestamps and relative windows like `past 6 hours`, `past 3 weeks`, or `past 2 years`, and the chart resolution options adapt to that chosen span instead of exposing the same fixed set for every custom window
- the full frontend Vitest run now uses forked workers with a default `50%` worker cap via `admin-ui/scripts/run-vitest.mjs` instead of forcing a single worker, while CI and coverage runs stay pinned to one worker for stability on stricter environments
- the Docker image build does not run the full Vitest suite by default; CI remains the always-on frontend test path, and you can opt into in-image frontend tests with `--build-arg RUN_TESTS=true` when you specifically want that extra validation step
- the top-level `App` and `RemoteApp` components now keep their real production polling/watchdog/anomaly timing defaults in explicit timing maps, and tests can inject shorter overrides so app-level live-poll and anomaly regressions do not need multi-second real waits
...
- destination IMAP setup now mirrors the newer source-account hardening flow: the dialog can preview folders directly from the current unsaved draft settings, `Test Connection` finishes first and then loads folders in the background, insecure non-TLS destinations show a visible warning, and InboxBridge automatically upgrades the draft to TLS plus locks the toggle when the destination server exposes either an implicit-TLS endpoint or STARTTLS support on the configured IMAP port
...
- reconnecting Gmail now uses a friendlier `Reconnect Gmail Account` action, warns before replacing the currently linked Gmail account, and the frontend Google OAuth callback route reports when the previous linked account was automatically replaced and its old grant revoked
...
- live polling notifications now follow the workspace that started the run: `Administration` starts keep the global-poll wording, while `My InboxBridge` and `InboxBridge Go` starts use the user-scoped wording even if the signed-in actor is an admin account
- broad user polling runs now keep only the run-level success/progress notifications (`Running polling for your mail accounts now…`, `Your polling run finished…`), while per-source success notifications are reserved for explicit single-source `Run Poll Now` actions; per-source failures still surface during broad runs so the failing account is visible
- mobile source-account rows and nested statistics cards should not force horizontal overflow; the current CSS keeps source cards, expanded configuration grids, and `My Statistics` cards on-screen by allowing inner content to wrap and by using grid columns that clamp to the section width
- mobile admin cards should also stay inside their parent sections, including the stacked Google/Microsoft OAuth cards and the deployment-mode plus per-user cards in `Users`, by clamping summary buttons and breakdown values instead of letting them grow past the right edge
- on narrow mobile widths, the `Administration -> OAuth Apps` summaries and `Users` card metadata should wrap into stacked rows inside their cards instead of staying on one long line, and each user card action menu should stay pinned in the top-right corner so the text can flow beneath it safely
- broad live polling runs should not duplicate their start notifications between optimistic UI actions and SSE updates; the controller now leaves run-start toasts to the live event stream, the explicit completion toast remains the final API result notification, and global completion copy should say that the global poll finished
- passkey cards should render the capability/status labels as separate stacked rows on mobile instead of squeezing `discoverable`, `backup eligible`, and `backed up` into one wrapped sentence
...
- on IMAP IDLE mailboxes, `Run Poll Now` still triggers an immediate one-off sync for that source without disabling the watcher or re-enabling scheduled polling
- source connection tests for IMAP mailboxes now surface a best-effort `$Forwarded` capability probe based on IMAP permanent flags so the UI can show whether the server appears to support that source-side marker
...
- those `Last result` summaries now keep all compact per-run counters as pills on both surfaces, including the persisted Spam/Junk mailbox count when present, instead of mixing pills with a separate explanatory sentence in `My InboxBridge`
...
- `AutocompleteInput` for anchored floating suggestion menus on text inputs that should behave like the pillbox dropdown without rebuilding keyboard navigation and viewport positioning from scratch; on narrow/coarse-pointer devices it keeps the text field editable and renders an attached dropdown directly below the input instead of switching to a native picker
...
- source email account dialog presets, auth-aware field visibility, and a reusable IMAP folder pillbox component with inline autocomplete, arrow-key suggestion navigation, direct text entry before folder discovery has been validated, green/red validation pills for known vs missing folders, viewport-aware floating suggestions that flip above or below the field and close if the anchor leaves the viewport, a coarse-pointer/mobile fallback to a native multi-select picker once server-known folders are locked in, and a save-warning modal that offers `Test Connection Now` or `Save Disabled Without Testing` when the current source settings have not been validated; once IMAP folders have been successfully loaded from the server, that pillbox stops accepting arbitrary folder names and only allows folders the server actually reported
