...
- the add/edit source-email-account dialog now groups its mailbox-processing controls more intentionally: `After polling` and `Mark as read after polling` share one row, `Source update mode` sits above the `TLS only` / `Unread only` row, and `Enabled` is the final toggle at the bottom of the form
