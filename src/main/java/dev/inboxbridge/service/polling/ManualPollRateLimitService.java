...
/**
 * Tracks recent manual poll invocations per actor so broad manual runs can be
 * rate-limited without affecting scheduled polling.
 */
@ApplicationScoped
public class ManualPollRateLimitService {
    ...
    public Decision tryAcquire(String actorKey, int maxRuns, Duration window, Instant now) {
        if (actorKey == null || actorKey.isBlank() || maxRuns < 1 || window.isZero() || window.isNegative()) {
