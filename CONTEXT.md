...
The backend mail-source path is now also split more intentionally: `MailSourceClient` still owns source polling orchestration, while dedicated helpers now own source-folder discovery/spam probing (`MailSourceFolderService`), fetched-message materialization/source-key handling (`MailSourceMessageMapper`), checkpoint-aware IMAP/POP candidate selection (`MailSourceCheckpointSelector`), mailbox connection plus OAuth-session retry/invalidation logic (`MailSourceConnectionService`), and source-side post-poll IMAP mutations such as mark-read / move / delete / `$Forwarded` handling (`MailSourcePostPollActionService`). Future cleanup in that area should keep protocol/session setup, folder discovery, checkpoint selection, MIME materialization, OAuth retry handling, and source-side mutations isolated instead of growing `MailSourceClient` back into one large all-purpose class.
