...
- both the main app and `/remote` now attempt device-location capture automatically once per new authenticated session instead of waiting for a button click, and the browser geolocation flow now retries with a lower-accuracy lookup when the first high-accuracy request fails
