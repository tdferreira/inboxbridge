...
- the OAuth browser callback pages now also use a narrower web feature seam under `dev.inboxbridge.web.oauth`: `GoogleOAuthCallbackPageRenderer`, `MicrosoftOAuthCallbackPageRenderer`, `OAuthPageI18n`, and `OAuthPageSupport` own the large HTML/JS callback pages plus shared localization/escaping helpers, while the REST resources stay focused on authorization, callback-state validation, and token-exchange control flow
