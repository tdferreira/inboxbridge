...
The admin UI still surfaces browser-extension access directly in `Security -> Sessions -> Browser extensions`, but the normal setup guidance is now extension-managed sign-in from the browser extension itself. The admin panel remains the place to review connected extensions, revoke stale installs, and issue a manual fallback token only if direct extension sign-in is not available for a specific browser profile.
