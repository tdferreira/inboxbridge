...
- the admin-ui Docker image no longer runs the full Vitest suite by default during `docker compose up --build`; CI remains the authoritative always-on frontend test path, while the Dockerfile keeps an opt-in `RUN_TESTS=true` build argument for explicit in-image validation because the full suite can hang after printing successful output inside container builds
