...
- the local Docker stack now publishes only the HTTPS frontend to the host by default; backend and PostgreSQL stay on the internal Docker network, backend/browser responses carry stricter security headers and CSP/HSTS protections, JDBC defaults to certificate-validated PostgreSQL TLS, and InboxBridge now rejects non-TLS source or destination mailbox configurations instead of saving insecure IMAP/POP3 settings
