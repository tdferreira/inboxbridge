...
Destination IMAP mailbox setup now mirrors that same source-account hardening flow. The destination dialog can preview folders from the current unsaved draft settings through a dedicated preview endpoint instead of relying only on the already-saved linked mailbox, `Test Connection` now completes first and then refreshes folders in the background, insecure non-TLS destination drafts show a visible warning card, and both destination connection tests plus saves automatically upgrade to TLS or reject plain IMAP whenever InboxBridge can verify either an implicit-TLS endpoint or STARTTLS support on the current IMAP port.
...
Gmail API destinations now support optional per-source folder-to-label mappings in addition to the existing default source label. UI-managed and env-managed source mailboxes store mappings as `folder=Gmail/Label` entries internally, separated by newlines in the admin UI payload or semicolons in `.env` through `MAIL_ACCOUNT_<n>__FOLDER_LABEL_MAPPINGS`, but the admin UI presents them as structured source-folder/Gmail-label picker rows with removable mapping pills rather than exposing the raw text format. During Gmail import, InboxBridge resolves labels from the fetched message's actual source folder, applies the default source label when present, applies the matching folder-specific label when present, avoids duplicate label IDs, and still relies on the existing Gmail label creation policy when a mapped label does not already exist. This mapping only changes Gmail destination labels; it does not alter source fetching, checkpoints, dedupe keys, or source-side post-poll actions.

IMAP sources now have an explicit spam/junk strategy. InboxBridge detects likely spam and junk folder names when folder lists are loaded, pre-fills editable multi-value source spam/junk folders in the admin UI, and stores `IGNORE`, `IMPORT_NORMAL`, or `IMPORT_AND_ROUTE` per source. `IMPORT_AND_ROUTE` adds every configured spam/junk source folder to the fetch list; Gmail destinations receive the `SPAM` system label instead of `INBOX`, while IMAP destinations append those messages to the configured destination spam/junk folder. Env-managed sources can use `MAIL_ACCOUNT_<n>__SPAM_JUNK_STRATEGY` and comma-separated `MAIL_ACCOUNT_<n>__SPAM_JUNK_SOURCE_FOLDERS`; the older singular `MAIL_ACCOUNT_<n>__SPAM_JUNK_SOURCE_FOLDER` remains a one-folder alias.

Admin configuration backups are two-tiered. `/api/admin/config-backups/safe` returns a redacted JSON snapshot and audit-logs the export. `/api/admin/config-backups/encrypted-secrets` is admin-only, requires the existing fresh sensitive-action reauthentication window, requires explicit risk acknowledgement, encrypts the full secret-bearing snapshot to an operator-provided RSA public key using RSA-OAEP-SHA256 plus AES-256-GCM, never stores the export server-side, and audit-logs only metadata including the public key fingerprint. The response warning must continue to state that anyone who decrypts the file can access configured mailboxes.

The source-account dialog's Gmail label pickers should use `/api/app/destination-config/gmail-labels` when the active destination is Gmail API, while retaining `INBOX` and `SPAM` as offline fallback options. This is intentionally a picker over Gmail label IDs, because Gmail API imports require label IDs rather than display names for user labels. Folder-label mapping rows should autocomplete source folders from the same detected-folder list used by the source folder pillbox whenever that list is available.
...
Source mailbox connection tests now finish independently from follow-up IMAP folder discovery. The UI keeps a successful connection test visible even if folder loading later times out, and folder-load failures surface as separate notifications instead of trapping the test button in a perpetual loading state.
User-managed source mailboxes now also reject semantic duplicates for the same user when the same server identity and effective credentials are already configured, across both password and OAuth2 source accounts.
The backend now also has a dedicated browser-extension auth/API slice. Extension requests still authenticate against persisted `extension_session` rows instead of reusing the main browser session cookie, but the extension can now sign in directly with the same username/password rules as the main app while using a browser-session handoff for passkey-backed accounts. InboxBridge issues short-lived extension access tokens plus rotating refresh tokens, stores only their hashes server-side, returns the canonical public browser URL derived from `PUBLIC_BASE_URL` or `PUBLIC_HOSTNAME` / `PUBLIC_PORT`, and keeps the extension API intentionally narrow: per-user status plus a user-scoped `run poll now` action only. That status payload is derived from durable poll history plus current live-poll state so failures that happened while the user was offline remain visible later without depending on browser notification history. Passkey-backed extension sign-in now starts a short-lived browser handoff request, finishes on the normal InboxBridge web origin after the user completes the regular web login or passkey ceremony there, and only then redeems extension-scoped rotating tokens from the extension side, which keeps Chrome and Firefox on the same UX without relying on WebAuthn inside the extension origin. Extension configuration now rejects non-HTTPS InboxBridge URLs, so extension auth, SSE, and poll requests always stay on encrypted transport.
The stronger secret-management work now also includes a deployment policy flag at `SECURITY_ALLOW_ENV_MANAGED_MAILBOX_SECRETS` (mapped as `inboxbridge.security.secret-management.allow-env-managed-mailbox-secrets`). It defaults to `true`, but when set to `false` InboxBridge ignores `MAIL_ACCOUNT_*` env-managed source definitions and the `.env` `GMAIL_REFRESH_TOKEN` fallback for the system Gmail destination, forcing mailbox secrets to come from the admin UI's encrypted database storage instead. The admin `Security -> Secret management` summary now surfaces whether env-managed mailbox secrets are still allowed, how many env-managed sources are configured in `.env`, and whether a `.env` Gmail refresh-token fallback is still present. That summary now also shows an explicit warning banner whenever plaintext mailbox secrets are still present in deployment config, including the case where policy already blocks those values but stale `.env` credentials remain, and flips to a success banner once the deployment no longer exposes any mailbox-secret fallback in env config.
System Gmail helper paths now also treat that policy consistently: dashboard/status code can distinguish `.env fallback blocked by policy` from a genuinely missing Gmail token, and Gmail label/import paths fail with an explicit operator-facing message when the deployment still has a blocked `.env` refresh token instead of a usable stored secret.
The repository now also includes a `browser-extensions/` workspace. Implemented targets now live under `browser-extensions/chromium/` and `browser-extensions/firefox/`, while cross-browser modules live under `browser-extensions/shared/` and the Safari target remains a placeholder for the later Apple packaging phase. The current extension implementation stays intentionally plain/static rather than a second React app: it uses lightweight popup/options/background entrypoints, shared browser-agnostic controllers, shared encrypted storage/API wrappers, browser-specific packaging scripts, runtime host-permission requests for the configured InboxBridge origin, and active-tab URL detection so the first-time setup path can usually prefill the InboxBridge public URL automatically. `tabs` access is now also runtime-optional rather than a mandatory install-time permission, so the current-tab detection helper asks only when the user explicitly uses that feature. The extension Settings UI now labels that helper as `Detect from active tabs`, and it explains that the button scans the current browser window for an already-open InboxBridge tab, copying that origin into the URL field and preferring the most recently used matching tab when several are open. The extension Settings page also exposes optional browser-notification toggles for grouped source-error alerts and successful manual polls started from the extension, and enabling either toggle must request the `notifications` permission at runtime instead of making it an install-time requirement. The extension icon set should stay aligned with the `/remote` PWA artwork, using the same `remote-icon.svg` source plus derived raster manifest icons. Popup refreshes should give visible in-progress and completion feedback, and the popup/settings pages should default to the signed-in InboxBridge theme while still allowing local overrides to `System`, `Light Green`, `Light Blue`, `Dark Green`, or `Dark Blue`. When the popup has no saved extension session, it should render a clear signed-out state and turn the primary action into a `Sign in` shortcut that opens Settings, while the Settings page remains the place that actually runs the username/password or passkey sign-in flow. Extension language and theme now default to the signed-in InboxBridge user preferences exposed by the extension auth/status payloads, while the Settings page can still override either one locally for that browser profile. Extension token refreshes are deliberately conservative: transient refresh transport failures keep the encrypted local auth bundle so the background worker can retry later, while invalid refresh-token responses re-check storage first to avoid deleting a newer token already rotated by another extension context; only a confirmed stale/invalid refresh token clears the saved sign-in. The extension background worker now also consumes `/api/extension/events` as an authenticated SSE stream through fetch-based parsing instead of plain `EventSource`, because the extension auth model requires bearer-token headers and should not move those tokens into query strings. Live polling events overlay the cached extension status immediately so the badge, popup, and toolbar icon can show a running poll, while source/run completion events still trigger a full status refresh so grouped error summaries and last-import totals remain authoritative. Manual polling from the popup uses an optimistic running overlay only while the backend trigger call is in flight; rejected/not-started trigger responses send a cancellation message to the background worker, while accepted responses mark the trigger as returned and force an immediate authoritative status refresh so quick completed or no-op runs let an idle backend status clear the toolbar instead of resurrecting synthetic running state. The toolbar presentation now uses both the action badge and a dynamic icon overlay on top of the shared InboxBridge artwork: polling shows blue, attention or transport failures show red, and signed-out state shows gray. Toolbar icon rendering is monotonic so a slower stale polling-icon render cannot overwrite a newer healthy/default state after status has already cleared. Shared browser-extension behavior should be tested once in the shared workspace, while each browser target carries only its own runtime-adapter and packaging assertions. The backend extension HTTP slice now also has packaged-runtime `@QuarkusIntegrationTest` coverage for its anonymous-access boundary.
The extension badge refresh path should only show the `?` error state when the actual status fetch fails. Non-critical follow-up work such as popup broadcasts or live-subscription reconnects must not overwrite an already successful badge refresh with a misleading error indicator.
The GitHub release workflow is now also expected to install each extension target's dependencies, run the Chromium and Firefox extension tests, package their distributables, and copy those `.zip` / `.xpi` artifacts into the release bundle alongside the backend and admin-ui assets. When store credentials are configured as repository secrets, that same workflow can also submit the packaged Chromium build to Chrome Web Store and Microsoft Edge Add-ons, and submit the Firefox build to AMO through `web-ext sign`; the one-time store-registration and secret setup steps are documented in `docs/BROWSER_EXTENSION_STORES.md`.
Token-safe browser-extension auth diagnostics now exist on both sides of the extension session boundary: the extension console records refresh attempts, stale-token recovery, transient failures, and saved-auth clearing decisions without raw access or refresh tokens, while the backend logs extension session creation, refresh rotation, revocation, and rejected access/refresh-token reasons with session metadata plus one-way token fingerprints. Backend refresh-token rotation uses a database row lock for the matched extension session so several extension contexts waking after browser or OS restart cannot rotate the same refresh token in parallel and immediately invalidate each other's newly saved tokens.
The admin UI still surfaces browser-extension access directly in `Security -> Sessions -> Browser extensions`, but the normal setup guidance is now extension-managed sign-in from the browser extension itself. The admin panel remains the place to review connected extensions, revoke one or all stale installs, and the Sessions tab now lets the user collapse the normal sessions card and browser-extensions card independently instead of adding a second nested tab row.
That browser-extension sessions list is intentionally current-focused: active extension access is separated from recently revoked extension sessions, `Revoke all extensions` marks every unrevoked extension session for the current user as revoked, and revoked sessions disappear from the normal Security panel after 30 days so long-lived installations do not leave dozens of obsolete revoked rows in day-to-day account review.
Repo extension work should now use the Codex skill `.codex/skills/inboxbridge-browser-extension-best-practices/SKILL.md`, which captures the current official Chrome, MDN, and Apple guidance for least-privilege permissions, encrypted storage, no remote code, target-specific testing, and packaging expectations.
...
That public browser URL is now resolved through one shared backend helper instead of ad hoc string assembly in multiple services, so OAuth redirect defaults and similar browser-facing links all follow the same precedence rules. Mailbox protocol session defaults are likewise centralized behind a shared mail-session factory plus typed Quarkus config mapping for connection and operation timeouts, which keeps IMAP, POP3, destination append, and IMAP IDLE code paths aligned without scattering hard-coded Jakarta Mail properties across several services.
...
The repository now also includes `.github/dependabot.yml` for weekly dependency update PRs across GitHub Actions, Maven, npm, and Dockerfiles, plus a `.github/workflows/release.yml` workflow that builds release assets and publishes GitHub Releases from tags matching `v*` or from a manual dispatch with a provided tag name. That release workflow now generates a reusable `Install And Artifacts` section for each GitHub Release, maps the main bundle, checksum, Chromium package, and Firefox package to the installation/setup docs stored in the repository at the exact released tag, and preserves release-specific commit history by keeping GitHub's generated notes plus an explicit compare/changelog link instead of relying only on autogenerated notes with no install guidance. The release bundle includes the source and Docker build inputs needed by its Compose file, and every release note now links directly to the canonical tagged upgrade section. The setup guide pairs custom-format PostgreSQL dumps with `.env` and generated-certificate archives, and documents the required order for recovery after a lost data volume: restore deployment configuration, start only PostgreSQL, restore the dump, then start the full stack. The admin UI embeds its `admin-ui/package.json` version at build time and displays it on both unauthenticated sign-in cards and in authenticated main-app Preferences, making an upgrade visible without a new API request or persistent dashboard chrome. Its browser-side GitHub latest-release check is cached for 24 hours per browser profile to avoid unnecessary API traffic.
The repository now also includes project-local Codex skills under `.codex/skills/` for common InboxBridge work: repo guardrails, backend changes, admin-ui changes, mailflow-safety work, validation/release handoff, and an optional high-skepticism mode for users who explicitly want stronger pushback than the repo's normal directness baseline. Those skills intentionally point back to `AGENTS.md`, `CONTEXT.md`, the repo docs, and the admin-ui README instead of duplicating long project references inside the skill bodies. `AGENTS.md` now also includes a portable AI-agent routing section plus an interaction-modes section so non-Codex agents can still follow the same repo workflow and communication posture just by reading that file.
Backend validation now intentionally includes a small Quarkus-managed component-test layer in addition to the long-standing plain JUnit and GreenMail coverage, so shared config-backed helpers can be exercised through CDI without paying the cost of turning the whole backend suite into full `@QuarkusTest` coverage.
Backend validation now also includes a packaged-runtime Quarkus smoke path through Maven Failsafe and `@QuarkusIntegrationTest`. That smoke layer currently covers the health endpoints, builds the test artifact with the H2 JDBC extension enabled for packaged runs, boots the jar under the `%test` profile instead of the normal Postgres/TLS Docker profile, and relies on test-only `@Vetoed` unit-test doubles so Arc bean discovery in packaged runs is not polluted by manual fake service subclasses from plain JUnit resource tests.
PostgreSQL- and transit-provider-backed `@QuarkusTestResource` declarations are restricted to their annotated test classes. Keep that scope explicit so Failsafe discovery cannot leak PostgreSQL JDBC URLs or Vault/OpenBao settings into the packaged H2 smoke runtime.
The backend mail-source path is now also split more intentionally: `MailSourceClient` still owns source polling orchestration, while dedicated helpers now own mailbox probing and connection-test flows (`MailSourceConnectionProbeService`), protocol fetch execution plus destination-aware checkpoint lookups (`MailSourceFetchService`), source-folder discovery/spam probing (`MailSourceFolderService`), fetched-message materialization/source-key handling (`MailSourceMessageMapper`), checkpoint-aware IMAP/POP candidate selection (`MailSourceCheckpointSelector`), mailbox connection plus OAuth-session retry/invalidation logic (`MailSourceConnectionService`), and source-side post-poll IMAP mutations such as mark-read / move / delete / `$Forwarded` handling (`MailSourcePostPollActionService`). Plain JVM callers now build that mail-source graph through `MailSourceStandaloneFactory` instead of each service re-creating its own fallback collaborators, so the non-CDI path used by focused tests stays aligned with the production dependency shape. Those extracted helpers now also use explicit constructor injection for their internal collaborators, which makes the dependency graph easier to read and gives Quarkus component tests a narrower seam for runtime wiring checks. Future cleanup in that area should keep protocol/session setup, fetch execution, probing, folder discovery, checkpoint selection, MIME materialization, OAuth retry handling, and source-side mutations isolated instead of growing `MailSourceClient` back into one large all-purpose class.
The backend polling path is now split more intentionally too: `PollingService` still owns run-level orchestration such as scheduler eligibility, live-run lifecycle, manual rate limits, and worker coordination, while `PollingSourceExecutionService` now owns the per-source execution pipeline that used to live inline inside `PollingService`. That extracted helper handles destination-link checks, spam/junk probing, dedupe-vs-import decisions, source/destination throttle accounting, live per-message progress updates, source checkpoint writes, and persisted poll-event / cooldown recording. Focused unit tests now exercise that seam directly, while the long-standing `PollingServiceTest` and GreenMail integration coverage still guard the higher-level polling behavior.
...
- self-registration now uses that same per-client-IP exponential lockout policy in addition to the CAPTCHA check, so repeated failed CAPTCHA solves, duplicate-username retries, or other bad registration attempts can also trigger a timed block from the same address
- the unauthenticated admin-ui login and self-registration forms now also apply a short frontend submit cooldown after each attempt so rapid repeated button presses do not hammer the browser-facing auth endpoints between backend lockout windows
...
- the same preferences model now also stores a per-user theme mode (`SYSTEM`, `LIGHT_GREEN`, `LIGHT_BLUE`, `DARK_GREEN`, `DARK_BLUE`), and both the main admin UI plus `/remote` should apply that saved mode while still resolving `SYSTEM` against the current browser color-scheme setting; older saved `LIGHT` / `DARK` values should be normalized forward to `LIGHT_GREEN` / `DARK_BLUE`
...
- the normal `cd admin-ui && npm run test:run` frontend suite now uses forked Vitest workers with a default `50%` worker cap instead of forcing one worker, while CI and coverage runs stay single-worker to remain more memory-stable on stricter environments
- the admin-ui Docker image installs the committed frontend lockfile with `npm ci`, excludes host `node_modules` and `dist` output from the build context, and no longer runs the full Vitest suite by default during `docker compose up --build`; CI remains the authoritative always-on frontend test path, while the Dockerfile keeps an opt-in `RUN_TESTS=true` build argument for explicit in-image validation because the full suite can hang after printing successful output inside container builds
...
- the local Docker stack now publishes only the HTTPS frontend to the host by default; backend and PostgreSQL stay on the internal Docker network, backend/browser responses carry stricter security headers and CSP/HSTS protections, JDBC defaults to certificate-validated PostgreSQL TLS, and both source plus destination IMAP mailbox configuration now follow a stricter `prefer TLS, allow plain only when neither implicit TLS nor STARTTLS can be verified` rule instead of blindly saving insecure settings
...
- backend polling-stat timeline bucketing now lives in `PollingTimelineService` rather than inline inside `PollingStatsService`, so the stats service can stay focused on repository scoping and event aggregation while the shared hourly/daily/weekly/monthly/custom bucket logic is regression-tested directly
...
Gmail import now also treats the specific `400 INVALID_ARGUMENT` response for an invalid attachment under Gmail policy 6590 as a message-local condition. Recognition covers the typed HTTP-boundary exception plus a strictly matched production-shaped generic or wrapped exception containing the Gmail import `400`, invalid-attachment text, and policy-6590 URL; this prevents exception translation at runtime boundaries from aborting the source batch while leaving unrelated Gmail `400` responses fatal. For an unsigned, unencrypted message, InboxBridge parses the raw MIME, removes directly prohibited attachments or a supported ZIP/TAR/GZip/BZip2 archive containing a prohibited filename, inserts a visible plain-text removal notice, marks the copy with `X-InboxBridge-Sanitized: gmail-invalid-attachment`, and retries the Gmail import exactly once. Archive inspection is recursive but bounded by depth, entry count, per-nested-archive size, and total expanded-byte limits; malformed or over-limit archives are left untouched. Any DKIM or ARC header, S/MIME signed/encrypted entity, PGP/MIME signed/encrypted entity, or protected attached message disables sanitization so the original raw bytes and cryptographic protection are never rewritten. If no safe sanitization is possible, or the one sanitized retry still fails, InboxBridge reports the skipped message, advances its source checkpoint, and continues with later mail. Successful sanitized imports keep dedupe identity based on the original raw MIME and persist `imported_message.content_sanitized`, which prevents source-side post-poll actions both on the initial import and on later duplicate replay so the original remains in the source mailbox. Other Gmail `400` responses and transient destination failures remain fatal for that source run so configuration or provider problems are still retried instead of silently discarding mail.
...
The admin UI now surfaces that same secret-management status inside its own
`Administration -> Secret management` section instead of burying it under
Authentication Security. That section shows the active mode/key, legacy-key
readiness, env-managed mailbox-secret policy state, and per-key usage summary.
`Re-encrypt stored secrets` now opens a dedicated high-friction modal with
...
access tokens can be cleared so clients must refresh through the newly trusted
...
That provider-backed coverage now also reaches the admin HTTP layer: a
PostgreSQL-backed Quarkus integration test boots live Vault/OpenBao transit
containers, seeds legacy local-key ciphertext into the real tables, and then
drives `/api/admin/secret-management`, `/report`, `/migration-guide`,
password re-auth, and `/re-encrypt` through the real browser-session auth flow
to prove the operator workflow works end-to-end on top of the same transit
providers.
...
for Flyway and repository/service validation. That Postgres-backed path now
also covers authenticated HTTP flows end-to-end: browser login plus persisted
`/api/app/ui-preferences`, browser-managed extension-session listing/revocation
...
- ALTCHA verification now uses the ALTCHA Java library's v2 proof-of-work flow end to end. The backend issues signed v2 challenges with structured `parameters` plus `signature`, the browser solves them by deriving keys from `nonce + counter`, and the submitted payload now follows the official `{ challenge, solution }` v2 shape instead of the older flat v1 payload fields. InboxBridge currently defaults that custom browser-side solver to a lighter ALTCHA v2 cost of `250` for responsiveness, the registration modal now shows an explicit anti-robot error message if solving fails instead of failing silently, and every shipped admin-ui locale now includes the ALTCHA/CAPTCHA-specific registration strings instead of falling back to English. ALTCHA verification still yields to the browser before solving so the shared loading spinner and `Verifying…` button state are visible during processing.
...
- processes eligible email accounts through a bounded virtual-thread worker pool
- now delegates one source at a time into `PollingSourceExecutionService`, which keeps per-source import/dedupe/throttle/checkpoint logic out of the run coordinator
...
- the add/edit source email account modal now groups mailbox-processing controls intentionally: `After polling` sits beside `Mark as read after polling`, `Source update mode` appears above the `TLS` / `Unread only` row, and `Enabled` is the final toggle
- that same source-account modal now treats source transport security as a verified policy rather than a static checkbox: users can disable TLS for legacy source servers, but the dialog shows a prominent warning for plain-text transport, the backend auto-tests both the standard implicit-TLS endpoint and STARTTLS/STLS support on the configured port during `Test Connection`, successful secure detection switches the form back to TLS automatically, and InboxBridge refuses to save a plain-text source config when it can prove that host already exposes an encrypted path
...
- Polling start/finish notifications are now scoped by the workspace trigger rather than only the actor role, so `My InboxBridge` and `InboxBridge Go` starts use the user-specific wording even when an admin account launched them, while `Administration` runs keep the global-poll wording.
- Broad user polling runs now suppress per-source success notifications from the live stream and keep only the run-level user notifications, while explicit single-source `Run Poll Now` actions still emit the per-source start/success notifications and per-source failures remain visible even during broad runs.
- On narrow mobile widths, the `Administration -> OAuth Apps` provider cards and the `Users` deployment/user cards should stay within their parent section width instead of expanding past the right edge; the shared card surfaces, summary buttons, muted boxes, and stat breakdown values now clamp and wrap accordingly.
- On those same narrow admin layouts, the OAuth summary copy and user metadata should break into stacked rows inside the card rather than rendering as one long clipped sentence, and the per-user action hamburger stays pinned to the top-right corner so the content can wrap cleanly underneath it.
- Live polling start notifications should come from the SSE/live-event pipeline rather than being duplicated by optimistic frontend toasts; broad user/admin runs keep their final API result notifications, but the transient `live poll finished` toast is intentionally suppressed, and global completion copy should explicitly say that the global poll finished.
- On mobile account security surfaces, passkey cards should render `discoverable`, `backup eligible`, and `backed up` as separate metadata rows for readability instead of a single wrapped line.
...
- Manual polling now preserves enabled user sources even when the user's destination mailbox is missing, records a per-source `destination_mailbox_not_configured` event, and returns a clear setup error in both My InboxBridge and browser extension manual runs; scheduler runs still skip destination-less user sources so background ticks do not create noisy history.
