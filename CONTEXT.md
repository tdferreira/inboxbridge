...
The backend mail-source path is now also split more intentionally: `MailSourceClient` still owns source polling orchestration and post-poll action flow, while dedicated helpers now own source-folder discovery/spam probing (`MailSourceFolderService`), fetched-message materialization/source-key handling (`MailSourceMessageMapper`), checkpoint-aware IMAP/POP candidate selection (`MailSourceCheckpointSelector`), and mailbox connection plus OAuth-session retry/invalidation logic (`MailSourceConnectionService`). Future cleanup in that area should keep protocol/session setup, folder discovery, checkpoint selection, MIME materialization, and OAuth retry handling isolated instead of growing `MailSourceClient` back into one large all-purpose class.
