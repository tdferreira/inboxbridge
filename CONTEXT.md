...
The backend mail-source path is now also split more intentionally: `MailSourceClient` still owns source polling orchestration, checkpoint selection, and mailbox connection flow, while dedicated helpers now own source-folder discovery/spam probing and fetched-message materialization/source-key handling. Future cleanup in that area should keep protocol/session setup, folder discovery, and MIME materialization isolated instead of growing `MailSourceClient` back into one large all-purpose class.
