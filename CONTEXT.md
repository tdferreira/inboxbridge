...
Gmail API destinations now support optional per-source folder-to-label mappings in addition to the existing default source label. UI-managed and env-managed source mailboxes can store mappings as `folder=Gmail/Label` entries, separated by newlines in the admin UI or semicolons in `.env` through `MAIL_ACCOUNT_<n>__FOLDER_LABEL_MAPPINGS`. During Gmail import, InboxBridge resolves labels from the fetched message's actual source folder, applies the default source label when present, applies the matching folder-specific label when present, avoids duplicate label IDs, and still relies on the existing Gmail label creation policy when a mapped label does not already exist. This mapping only changes Gmail destination labels; it does not alter source fetching, checkpoints, dedupe keys, or source-side post-poll actions.

IMAP sources now have an explicit spam/junk strategy. InboxBridge detects likely spam/junk folder names when folder lists are loaded, pre-fills editable source and destination spam/junk paths in the admin UI, and stores `IGNORE`, `IMPORT_NORMAL`, or `IMPORT_AND_ROUTE` per source. `IMPORT_AND_ROUTE` adds the configured spam/junk source folder to the fetch list; Gmail destinations receive the `SPAM` system label instead of `INBOX`, while IMAP destinations append those messages to the configured destination spam/junk folder. Env-managed sources can use `MAIL_ACCOUNT_<n>__SPAM_JUNK_STRATEGY` and `MAIL_ACCOUNT_<n>__SPAM_JUNK_SOURCE_FOLDER`.
...
The source-account dialog's Gmail label picker should use `/api/app/destination-config/gmail-labels` when the active destination is Gmail API, while retaining `INBOX` and `SPAM` as offline fallback options. This is intentionally a picker over Gmail label IDs, because Gmail API imports require label IDs rather than display names for user labels.
