...
- the local Docker stack now publishes only the HTTPS frontend to the host by default; backend and PostgreSQL stay on the internal Docker network, backend/browser responses carry stricter security headers and CSP/HSTS protections, JDBC defaults to certificate-validated PostgreSQL TLS, destination mailbox configuration still requires TLS, and source mailboxes now follow a stricter `prefer TLS, allow plain only when no implicit TLS endpoint can be verified` rule instead of blindly saving insecure IMAP/POP3 settings
...
- that same source-account modal now treats source transport security as a verified policy rather than a static checkbox: users can disable TLS for legacy source servers, but the dialog shows a prominent warning for plain-text transport, the backend auto-tests the standard secure IMAP/POP3 endpoint during `Test Connection`, successful secure detection switches the form back to TLS automatically, and InboxBridge refuses to save a plain-text source config when it can prove that host already exposes an implicit TLS endpoint
