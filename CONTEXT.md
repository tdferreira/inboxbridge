...
- the admin `Global Statistics` section now computes a client-side anomaly warning from the hourly scheduled-run timeline plus the current effective polling interval and enabled-source count; if the observed scheduled runs in an hour are dramatically higher than that interval would allow, the section emits a linked warning notification for administrators, pulses red only temporarily to draw attention, and then stays in a static warning state so past scheduler bugs remain visible in the UI instead of being filtered out of the history
